...
| `pto.cmo.cacheinvalid %addr single_cache_line : !pto.ptr<T, gm>` | 指定 GM 地址所在 cache line 的 cache maintenance 边界 | `dcci(addr, cache_line_t::SINGLE_CACHE_LINE)` |
...
`all` 表示 whole-cache 粒度；`single_cache_line` 表示只处理 `%payload_ptr` 所在
cache line。当前还没有 range 形式；如果 payload 横跨多条 cache line，PyPTO 或用户需要
...
- `pto.cmo.cacheinvalid %addr single_cache_line : !pto.ptr<T, gm>` lower 到 `dcci((__gm__ void*)addr, cache_line_t::SINGLE_CACHE_LINE)`。
