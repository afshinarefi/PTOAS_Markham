...
namespace mlir {
namespace pto {
#define GEN_PASS_DEF_FUSIONPLANVERSIONSELECTION
...
struct FusionImplementationSignature {
  unsigned loopDepth;
  pto::FusionVFImplKind vfImplKind;

  bool operator==(const FusionImplementationSignature &other) const {
    return loopDepth == other.loopDepth && vfImplKind == other.vfImplKind;
  }
};

struct PlannedFusionGroup {
  SmallVector<const pto::FusionComputeNode *, 8> members;
  std::optional<FusionImplementationSignature> implementation;
};
...
struct PlanningCost {
  ...
  bool rejectedForImplementationMismatch = false;
...
static bool hasImplementation(const pto::FusionComputeNode &node,
                              const FusionImplementationSignature &signature) {
  if (node.semantics.versions.empty())
    return true;
  return llvm::any_of(node.semantics.versions, [&](const auto &version) {
    return version.loopDepth == signature.loopDepth &&
           version.vfImplKind == signature.vfImplKind;
  });
}

static std::optional<FusionImplementationSignature>
selectGroupImplementation(ArrayRef<const pto::FusionComputeNode *> members) {
  const pto::FusionComputeNode *firstVersioned = nullptr;
  for (const pto::FusionComputeNode *member : members) {
    if (!member->semantics.versions.empty()) {
      firstVersioned = member;
      break;
    }
  }
  if (!firstVersioned)
  ...
  for (const pto::FusionTileOpVersions &version :
       firstVersioned->semantics.versions) {
    // Post-update implementations carry address state across iterations. Until
    // fusion models and rewrites that state explicitly, they are not legal
    // implementations for a fusion group.
    if (version.vfImplKind != pto::FusionVFImplKind::NoPostUpdate)
      continue;

    FusionImplementationSignature candidate{version.loopDepth,
                                            version.vfImplKind};
    if (!llvm::all_of(members, [&](const pto::FusionComputeNode *member) {
          return hasImplementation(*member, candidate);
        }))
      continue;

    // Candidate order is the implementation preference order. Select the
    // first no-post-update signature supported by every member.
    return candidate;
  }
  return std::nullopt;
}
...
static bool
groupHasVersionedMember(ArrayRef<const pto::FusionComputeNode *> members) {
  return llvm::any_of(members, [](const pto::FusionComputeNode *member) {
    return !member->semantics.versions.empty();
  });
}
...
static void assignStableGroupMetadata(ArrayRef<PlannedFusionGroup> groups,
  ...
  for (const PlannedFusionGroup *group : orderedGroups) {
    ...
    for (auto [order, node] : llvm::enumerate(stableOrder)) {
      ...
      const FusionImplementationSignature &implementation =
          *group->implementation;
      auto selectedVersion = llvm::find_if(
          node->semantics.versions,
          [&](const pto::FusionTileOpVersions &version) {
            return version.loopDepth == implementation.loopDepth &&
                   version.vfImplKind == implementation.vfImplKind;
          });
      assert(selectedVersion != node->semantics.versions.end() &&
...
public:
  ...
  evaluateAppend(const PlanningContext &ctx,
    ...
    decision.implementation = selectGroupImplementation(proposedGroup);
    if (groupHasVersionedMember(proposedGroup) && !decision.implementation) {
      decision.cost.rejectedForImplementationMismatch = true;
      return decision;
    }
...
public:
  ...
  planBlock(const PlanningContext &ctx,
    ...
    auto flushChain = [&]() {
      ...
      group.members = chain;
      group.implementation = selectGroupImplementation(group.members);
...
// Fusion groups select a common NoPostUpdate implementation above. Every
// remaining versioned TileOp still needs a concrete standalone implementation
// so that no candidate list survives this pass. The producer-defined versions
// order is the standalone preference order.
static LogicalResult selectStandaloneVersions(func::FuncOp func,
                                              MLIRContext *ctx) {
  WalkResult result = func.walk([&](Operation *op) {
    if (!op->hasAttr(kVersionsAttr))
      return WalkResult::advance();

    FailureOr<SmallVector<pto::FusionTileOpVersions>> versions =
        pto::getFusionTileOpVersions(op);
    if (failed(versions)) {
