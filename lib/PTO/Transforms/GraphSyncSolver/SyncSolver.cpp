...
static Operation *getMultiBufferAnchorOpInLoop(RWOperation *rwOp,
  ...
  if (!loopOp || rwOp->op == loopOp || !loopOp->isProperAncestor(rwOp->op))
    return nullptr;
  return rwOp->op;
}

static std::pair<Operation *, Operation *>
getMultiBufferWaitSetAnchors(ConflictPair *conflictPair) {
  assert(conflictPair != nullptr);
  if (!conflictPair->eventIdInfo.isMultiBuffer())
    return {nullptr, nullptr};

  Operation *anchor1 = getMultiBufferAnchorOpInLoop(
      conflictPair->op1, conflictPair->eventIdInfo.multibufferLoop);
  Operation *anchor2 = getMultiBufferAnchorOpInLoop(
      conflictPair->op2, conflictPair->eventIdInfo.multibufferLoop);
...
SyncBeforeAfterMap Solver::getBeforeAfterSyncMaps() {
  ...
  for (auto *conflictPair : conflictPairs) {
    ...
    if (conflictPair->isBarrier()) {
      ...
      auto [waitDynAnchor, setDynAnchor] =
          getMultiBufferWaitSetAnchors(conflictPair);
      waitOp->multiBufferDynAnchor = waitDynAnchor;
      setOp->multiBufferDynAnchor = setDynAnchor;
