...
@pto.inline_proc
def _part_add(dst, full, part, dst_rows, dst_cols, part_rows, part_cols):
...
@pto.tile_template(op="pto.tpartadd", target="a5", name="template_tpartadd")
def template_tpartadd(src0: pto.Tile, src1: pto.Tile, dst: pto.Tile):
    ...
    if src0_rows == dst_rows:
        if src0_cols == dst_cols:
            _part_add(dst, src0, src1, dst_rows, dst_cols, src1_rows, src1_cols)
    elif src1_rows == dst_rows:
        if src1_cols == dst_cols:
            _part_add(dst, src1, src0, dst_rows, dst_cols, src0_rows, src0_cols)
