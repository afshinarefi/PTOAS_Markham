    ...
    for row in range(0, valid_rows, 1):
        ...
        for col in range(0, valid_cols, pto.get_lanes(dtype)):
            ...
            if pto.constexpr(str(dtype) == "f32"):
                quotient = pto.vtrc(pto.vdiv(lhs, rhs, mask), mask, rnd="Z")
                result = pto.vsub(lhs, pto.vmul(quotient, rhs, mask), mask)
            elif pto.constexpr(str(dtype) == "f16"):
                ...
                result_even = pto.vsub(
                    lhs_even,
                    pto.vmul(
                        pto.vtrc(pto.vdiv(lhs_even, rhs_even, mask), mask, rnd="Z"),
                        rhs_even,
                        mask,
                    ),
                    mask,
                ...
                result_odd = pto.vsub(
                    lhs_odd,
                    pto.vmul(
                        pto.vtrc(pto.vdiv(lhs_odd, rhs_odd, mask), mask, rnd="Z"),
                        rhs_odd,
                        mask,
                    ),
                    mask,
