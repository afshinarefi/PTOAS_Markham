...
def template_tfmods(src: pto.Tile, scalar: pto.Scalar, dst: pto.Tile):
    ...
    if pto.constexpr(str(dtype) == "f16"):
        ...
        for row in range(0, valid_rows, 1):
            ...
            for col in range(0, valid_cols, pto.get_lanes(pto.f16)):
                ...
                quotient_even = pto.vtrc(pto.vdiv(even, scalar_f32, mask32), mask32, rnd="Z")
                quotient_odd = pto.vtrc(pto.vdiv(odd, scalar_f32, mask32), mask32, rnd="Z")
        ...
        for row in range(0, valid_rows, 1):
            ...
            for col in range(0, valid_cols, pto.get_lanes(dtype)):
                ...
                vec = pto.vlds(src[row, col:])
                quotient = pto.vdiv(vec, pto.vbr(scalar), mask)
                if pto.constexpr(str(dtype) == "f32"):
                    quotient = pto.vtrc(quotient, mask, rnd="Z")
                result = pto.vsub(vec, pto.vmuls(quotient, scalar, mask), mask)
