...
@pto.tile_template(op="pto.trowargmin", target="a5", name="template_trowargmin")
def template_trowargmin(src: pto.Tile, tmp: pto.Tile, dst: pto.Tile):
    ...
    for row in range(0, valid_rows, 1):
        remained = valid_cols
        value_acc = pto.vbr(pto.PadValue.MAX.eval(src_dtype))
        ...
        for col in range(0, valid_cols, lanes):
            ...
            reduced = pto.vcmin(pto.vlds(src[row, col:]), mask)
            ...
            selected = pto.vcmp(value_acc, value, value_mask, "gt")
